...
const PLAN_LIMITS = {
    free:  { scans: 4,        ordersPerMonth: 1,        label: 'Free Trial' },
...
(async function syncPlanOnLoad() {
    ...
    try {
        ...
        if (data.success) {
            ...
            session.unlimitedScans       = data.unlimitedScans;
...
function checkScanLimit() {
    ...
    if (user.unlimitedScans) return { allowed: true };
...
function showUnlockModal(message) {
    ...
    modal.innerHTML = `
        <div style="background:white;border-radius:20px;padding:2.5rem;max-width:440px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            <div style="font-size:3rem;margin-bottom:1rem;">🔓</div>
            <h2 style="margin:0 0 0.75rem;color:#1a202c;font-size:1.4rem;">Free Scans Used Up</h2>
            <p style="color:#4a5568;margin:0 0 1.5rem;line-height:1.6;">${message}</p>
            <div style="display:flex;flex-direction:column;gap:0.75rem;">
                <button id="unlockBuyBtn" style="display:block;padding:12px 24px;background:linear-gradient(135deg,#10b981,#059669);color:white;border:none;border-radius:10px;font-weight:700;font-size:15px;cursor:pointer;">Unlock Unlimited Scans — $10 One-Time</button>
                <button onclick="document.getElementById('unlockModal').remove()" style="padding:10px;background:white;color:#718096;border:1px solid #e2e8f0;border-radius:10px;cursor:pointer;font-size:14px;">Maybe Later</button>
            </div>
        </div>`;
    modal.querySelector('#unlockBuyBtn').addEventListener('click', startScanUnlockCheckout);
    modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
    document.body.appendChild(modal);
}

async function startScanUnlockCheckout() {
    const btn = document.getElementById('unlockBuyBtn');
    if (btn) { btn.textContent = 'Loading...'; btn.disabled = true; }
    try {
        const session = subGetUser();
        if (!session.token) { window.location.href = 'login.html'; return; }
        const res = await fetch('https://gardiy-backend-production.up.railway.app/api/create-unlock-session', {
            method: 'POST',
        ...
        if (data.success && data.url) {
            ...
            alert('Could not start checkout: ' + (data.message || 'Unknown error'));
            if (btn) { btn.textContent = 'Unlock Unlimited Scans — $10 One-Time'; btn.disabled = false; }
        }
    } catch (err) {
