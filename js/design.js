...
function buildProductItemsHTML(products, esc) {
    ...
    return products.map(p => {
        ...
        const outOfSeasonClass = isOutOfSeason(p) ? ' out-of-season' : '';
        return `<div class="product-item${outOfSeasonClass}" data-pid="${p._pid}">
            ${thumb}
            <div class="product-info">
                <div class="product-name">${esc(p.name)}${sunBadgeHTML(p.name, p.category, p.sun)}${LARGE_TREES.has(p.name) && (p.category||'').toLowerCase()==='trees' ? '<span class="large-tree-badge" title="Large tree (40 ft+)">XL</span>' : ''}${outOfSeasonBadgeHTML(p)}</div>
...
function isOutOfSeason(product) {
    const seasons = product && product.seasons;
    if (!Array.isArray(seasons) || !seasons.length) return false; // year-round item (evergreen, hardscape, etc.)
...
function sunBadgeHTML(name, category, sunField) {
    ...
    if (req === 'shade')    return `<svg class="sun-badge" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" title="Shade" aria-label="Shade"><path fill="#CBD5E1" d="M29 28H9A9 9 0 0 1 9 10a9 9 0 0 1 3.5.7A10 10 0 0 1 34 20a6 6 0 0 1-5 8z"/><path fill="#fff" d="M28 26.5H10a7.5 7.5 0 0 1 0-15 7.5 7.5 0 0 1 3 .6A8.5 8.5 0 0 1 32.5 20a5.5 5.5 0 0 1-4.5 6.5z"/></svg>`;
    return '';
}
...
async function loadProductCategories() {
    ...
    const categories = {
        ...
        ornamental:   { name: 'Ornamental Grasses', icon: '🌾', iconImg: 'images/categories/ornamental.png?v=2',  products: [] },
        ...
        groundcovers: { name: 'Ground Covers',      icon: '🍀', iconImg: 'images/categories/groundcovers.png?v=2', products: [] },
...
async function addItemToCanvas(itemData, x, y, customW, customH) {
    ...
    placedItems.push({
        ...
        type: itemData.type, price: priceVal, seasons: itemData.seasons || [],
    });
    ...
    if (anyColored && !document.getElementById('plantLegend')) {
        ...
        if (h3) {
            ...
            legend.innerHTML = `
                ...
                <span class="legend-item sun-legend">
                    <span class="sun-legend-pair"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36"><path fill="#FF9800" d="M17 1.5h2v5h-2zm0 28h2v5h-2zM1.5 17h5v2h-5zm28 0h5v2h-5zM6.9 5.5l1.4-1.4 3.5 3.5-1.4 1.4zm17 17l1.4-1.4 3.5 3.5-1.4 1.4zM5.5 29.1l3.5-3.5 1.4 1.4-3.5 3.5zm17-17l3.5-3.5 1.4 1.4-3.5 3.5z"/><circle fill="#FFD600" cx="18" cy="18" r="9.5"/><circle fill="#FFF176" cx="15.5" cy="15.5" r="3.5"/></svg>Full sun</span>
                    <span class="sun-legend-pair"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36"><circle fill="#FFD600" cx="13" cy="13" r="8"/><circle fill="#FFF176" cx="11" cy="11" r="3"/><path fill="#E3F2FD" d="M28 30H15A8 8 0 0 1 15 14a8 8 0 0 1 3 .6A9 9 0 0 1 34 23a5 5 0 0 1-6 7z"/><path fill="#fff" d="M27 28.5H16a6.5 6.5 0 0 1 0-13 6.5 6.5 0 0 1 2 .3A7.5 7.5 0 0 1 32.5 23a4.5 4.5 0 0 1-5.5 5.5z"/></svg>Both</span>
                    <span class="sun-legend-pair"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36"><path fill="#CBD5E1" d="M29 28H9A9 9 0 0 1 9 10a9 9 0 0 1 3.5.7A10 10 0 0 1 34 20a6 6 0 0 1-5 8z"/><path fill="#fff" d="M28 26.5H10a7.5 7.5 0 0 1 0-15 7.5 7.5 0 0 1 3 .6A8.5 8.5 0 0 1 32.5 20a5.5 5.5 0 0 1-4.5 6.5z"/></svg>Shade</span>
                </span>
            `;
